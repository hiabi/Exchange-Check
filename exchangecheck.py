# Versión final corregida con retry automático y doble estrategia de ciclos
...
if st.button("🧮 Find Exchange Cycles"):
    ...
    st.subheader("🔁 Greedy Matching Cycles")
    cycles_greedy = sample_cycles_greedy(G, request_map)
    df_greedy, _ = describe_cycles(cycles_greedy, request_map)
    st.dataframe(df_greedy.head(10))

    st.subheader("🧠 Exhaustive Matching Cycles")
    cycles_exhaustive = sample_cycles_exhaustive(G, request_map)
    df_exhaustive, _ = describe_cycles(cycles_exhaustive, request_map)
    st.dataframe(df_exhaustive.head(10))

    output = BytesIO()
    df_exhaustive.to_csv(output, index=False)
    st.download_button("📥 Download Exhaustive Cycles", data=output.getvalue(), file_name="exchange_cycles.csv", mime="text/csv")
